...
def load_planar_dataset(seed):
    ...
    Y = np.zeros((m, 1),
                 dtype='uint8')  # labels vector (0 for red, 1 for blue)
...
def update_parameters(parameters, grads, learning_rate):
    ...
    for k in range(n):
        parameters["W" + str(k + 1)] = parameters[
            "W" + str(k + 1)] - learning_rate * grads["dW" + str(k + 1)]
        parameters["b" + str(k + 1)] = parameters[
            "b" + str(k + 1)] - learning_rate * grads["db" + str(k + 1)]
...
def load_dataset():
    ...
    train_set_x_orig = train_set_x_orig.reshape(train_set_x_orig.shape[0],
                                                -1).T
...
def load_planar_dataset(randomness, seed):
    ...
    for j in range(2):
        ...
        ix = range(N * j, N * (j + 1))
        if j == 0:
            t = np.linspace(j, 4 * 3.1415 * (j + 1),
                            N)  #+ np.random.randn(N)*randomness # theta
            r = 0.3 * np.square(t) + np.random.randn(N) * randomness  # radius
        if j == 1:
            t = np.linspace(j, 2 * 3.1415 * (j + 1),
                            N)  #+ np.random.randn(N)*randomness # theta
